function [x_crb,y_crb,z_crb,m_crb,z_stack] = CRB_zp(nPoints,xp,yp,zf,s,range)
...
z_stack = linspace(0, range*1e-6, nPoints);
...
for i = 1:nPoints
    ...
    origInt = PSF(xp, yp, zp, s);
    
    newInt_x = PSF(xp + delta.x, yp , zp, s);
    newInt_y = PSF(xp, yp + delta.y, zp, s);
    newInt_z = PSF(xp, yp, zp + delta.z, s);
  
    s.mass = s.mass + delta.m;
    newInt_m = PSF(xp, yp, zp, s);
    s.mass = s.mass - delta.m;
