...
% Factors with theta and r_d, with the Bessel function
% size (nThetas, s.cam_pixels^2)
% bessel_arg = s.k * s.ni * reshape(sin(thetas), nThetas, 1) * ...
%     reshape(r_d, 1, s.cam_pixels^2);
bessel_arg = s.ni * s.k * reshape(sin(thetas), nThetas, 1) * ...
    reshape(r_d, 1, s.cam_pixels^2);
...
N = sum(sum(abs(E_s1).^2 + abs(E_s2).^2));
